  ...
  return (
    <div className="border border-zus-grey-300 rounded overflow-hidden mb-4">
      <button
        type="button"
        onClick={onToggle}
        className={`w-full px-6 py-4 transition-colors flex items-center justify-between cursor-pointer ${
          isOpen
        ...
        <div className="flex items-center gap-3">
          ...
          <h3
            className={`text-xl font-bold text-left ${
              isOpen ? "text-zus-green" : "text-zus-grey-900"
            }`}
          >
            {title}
          </h3>
          {badge && (
            <span
              className={`text-xs px-2 py-1 rounded font-semibold ${
                badge.includes("✓") || badge.includes("WYPEŁNIONE")
