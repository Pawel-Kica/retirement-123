...
export function Input({
    ...
    return (
        <div className="w-full">
            {label && (
                <label className="block text-sm md:text-base font-medium text-zus-grey-700 mb-2">
                    {label}
                    {props.required && <span className="text-zus-error ml-1">*</span>}
                </label>
            )}
            <div className="relative">
                ...
                />
                {suffix && (
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-zus-grey-500 text-sm">
                        {suffix}
                    </span>
                )}
            </div>
