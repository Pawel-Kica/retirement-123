...
import { YearVisual } from "@/components/ui/YearVisual";
...
export default function SimulacjaPage() {
  ...
  const handleChange = (field: keyof SimulationInputs, value: any) => {
    ...
    if (field === "workStartYear" && value !== undefined) {
      ...
      } else if (formData.age && value < currentYear - formData.age + 15) {
        setErrors((prev) => [
          ...prev,
          {
            field: "workStartYear",
            message:
              "Rok rozpoczęcia pracy nie pasuje do podanego wieku (za wcześnie - musisz mieć co najmniej 15 lat)",
          },
        ]);
      }
    }
  ...
  const validateSection1 = (): boolean => {
    ...
    if (!formData.workStartYear) {
      ...
      formData.age &&
      formData.workStartYear < currentYear - formData.age + 15
    ) {
      sectionErrors.push({
        field: "workStartYear",
        message:
  ...
  const startYearOptions: Array<{ value: number | string; label: string }> = [
  ...
  const endYearOptions: Array<{ value: number | string; label: string }> = [
    { value: "", label: "Wybierz rok..." },
  ];
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="mt-6 pt-6 border-t border-zus-grey-300">
              <div className="mb-4 p-4 bg-blue-50 border-l-4 border-zus-navy rounded">
                <p className="text-sm text-center text-zus-grey-700">
                  💡 Te pola są opcjonalne. Możesz je pominąć, jeśli nie znasz
                  wartości.
                </p>
              </div>
          ...
          >
            ...
            <div className="mt-6 pt-6 border-t border-zus-grey-300">
              <div className="mb-4 p-4 bg-blue-50 border-l-4 border-zus-navy rounded">
                <p className="text-sm text-center text-zus-grey-700">
                  💡 Ta sekcja jest opcjonalna. Możesz włączyć lub wyłączyć
                  uwzględnienie L4.
                </p>
              </div>
              <Button
              ...
              {isSection1Complete() && (
                <div className="p-4 bg-zus-green-light border-l-4 border-zus-green rounded">
                  <p className="text-sm text-center text-zus-green font-semibold">
                    ✅ Wszystkie wymagane dane wypełnione! Możesz teraz
                    zaprognozować emeryturę.
                  </p>
                </div>
              )}
            </div>
            ...
            {!isSection1Complete() && (
              <div className="mb-4 p-4 bg-orange-50 border-l-4 border-zus-warning rounded">
                <p className="text-sm text-center font-semibold text-zus-grey-900">
                  ⚠️ Uzupełnij sekcję "Dane podstawowe", aby móc zaprognozować
                  emeryturę
                </p>
              </div>
            )}
