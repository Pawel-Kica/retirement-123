 ...
 */
export async function calculateDeferralScenarios(
  params: CalculateDeferralScenariosParams
): Promise<DeferralScenario[]> {
  const {
    baseRetirementYear,
    baseRetirementAge,
    baseTotalCapital,
  ...
  for (const additionalYears of deferralYears) {
    ...
    if (extendedPath.length === 0) {
      console.warn(
        `Brak danych płac do roku ${newRetirementYear}, pomijam scenariusz +${additionalYears}`
      );
      continue;
    }
    ...
    const newNominalPension = await calculatePension(
      newTotalCapital,
      newRetirementAge,
      sex
    );
