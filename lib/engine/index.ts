...
export async function calculateSimulation(
    ...
    const retirementAge = currentYear + (data.retirementAge[inputs.sex] - inputs.age);
    ...
    const nominalPension = inputs.includeL4 ? nominalPensionWithL4 : nominalPensionWithoutL4;
    const realPension = inputs.includeL4 ? realPensionWithL4 : realPensionWithoutL4;
    const capitalPath = inputs.includeL4 ? capitalPathWithL4 : capitalPathWithoutL4;
    ...
    const avgPensionInRetirementYear = data.avgPension[inputs.workEndYear.toString()] as number;
    ...
    const deferrals = calculateDeferralScenarios({
        ...
        baseTotalCapital: inputs.includeL4 ? finalCapitalWithL4.total : finalCapitalWithoutL4.total,
        baseRealPension: realPension,
        completeSalaryPath: inputs.includeL4
            ? calculateL4Comparison(completeSalaryPath, inputs.sex, data.sickImpactM, data.sickImpactF).withL4
            : completeSalaryPath,
        sex: inputs.sex,
